...
import { formatCurrency, formatNumber, formatGainLoss, cn } from '../utils/format';
...
export default function PositionCard({ position, onClick, className, style }: PositionCardProps) {
  ...
  return (
    <Card hover={isClickable} className={cn(className, isClickable && 'cursor-pointer')} style={style}>
      <div className="space-y-4" onClick={onClick} role={isClickable ? 'button' : undefined} tabIndex={isClickable ? 0 : undefined}>
        ...
        <div className="pt-2 border-t">
          <div className="flex justify-between items-center">
            ...
            <span className={cn('font-semibold', gainLossColor)}>{gainLossFormatted}</span>
          </div>
        </div>
      </div>
    </Card>
